...
export default function NewProductPage() {
  ...
  useEffect(() => {
    ...
    if (savedBrands.length > 0 && !formData.brand) {
      setFormData(prev => ({ ...prev, brand: savedBrands[0].name }));
    }
  }, []);
  const colors = [
  ...
  const handleSubmit = (e) => {
    ...
    // Agregar al array
    const updatedProducts = [...customProducts, newProduct];
    localStorage.setItem("customProducts", JSON.stringify(updatedProducts));

    // También agregar inventario inicial
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Completa los detalles del nuevo producto</p>
      </div>
